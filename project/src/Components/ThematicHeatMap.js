...
const ThematicHeatMap = ({ isLoadingThematicHeatMap, setIsLoadingThematicHeatMap }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch(`https://stock-api-0mm8.onrender.com/getthematicheatmapdata`);
            if (!response.ok) {
    ...
    return (
        <Box>
            {
                isLoadingThematicHeatMap ? <Box sx={{ height: "30rem", display: "flex", justifyContent: "center", alignItems: "center" }}><CircularProgress /> </Box> :
                    <Box>
                        <Typography sx={{ pl: 5, fontWeight: "bold", color: "blue" }}>Displaying All Thematic Stocks</Typography>
                        <Heatmap
